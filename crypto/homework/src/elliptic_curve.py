# We DO think you need to worry too much about this file.
...
class EC:
    ...
    def iszeropoint(self, p):
        ...
        dx = self.dfdx(x,y)
        dy = self.dfdy(x,y)
        if not dx.is_unit() and not dy.is_unit() and gcd(dx,dy)!=1:
            print("This point is singular!")
            return False
        return self.f(x, y) == self.k(0)

    def negate(self, p):
        if not self.iszeropoint(p):
            return EC.O
        if p == EC.O:
            return EC.O
        x, y = p
    ...
    def add(self, p1, p2):
        if not (self.iszeropoint(p1) and self.iszeropoint(p2)):
            return EC.O
        if p1 == EC.O:
            return p2
        if p2 == EC.O:
